...
// 安全な加算: 手動の ±4×10^18 境界ではなく、2の補数ラップ意味論から
// 導かれる厳密なオーバーフロー非発生条件を requires に置く。
// `((a+b) ^ a) & ((a+b) ^ b) < 0` は「和の符号が両オペランドの符号と
// 食い違った」= 桁あふれ、の標準的な検出式であり、その否定が
// 「a + b が i64 で表現可能」と同値（BV(64) 意味論で厳密）。
atom safe_add(a: i64, b: i64)
    requires: ((a + b) ^ a) & ((a + b) ^ b) >= 0;
    ensures: result == a + b;
    body: {
        a + b
    };

// 安全な乗算: 手動の ±2×10^9 境界ではなく、ラップ後の積から復元できるか
// で判定する厳密な条件。オーバーフローしていなければ p = a*b は
// p / a == b かつ p / b == a を満たし、逆にラップした場合はいずれかが
// 崩れる（a = MIN, b = -1 のような bvsdiv 飽和ケースも第2連言が検出する）。

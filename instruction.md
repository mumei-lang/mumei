...
**Subsumption check**: When calling `apply(5, atom_ref(increment))`, the verifier checks both directions of the callback contract. The declared callback precondition must imply the concrete atom's precondition, `contract.requires ⇒ concrete.requires`, and the concrete atom's postcondition must imply the declared callback postcondition under its precondition, `(concrete.requires ∧ concrete.ensures) ⇒ contract.ensures`. Contract clauses are checked using only positional callback aliases (`arg{i}`, `x`, `y`, `len_*`, and `result`); references to other callee parameters are unconstrained and therefore fail closed. If either implication is violated, or Z3 cannot decide it, verification fails closed with an error. A precondition violation reports `Contract subsumption failed: atom_ref(foo) passed to apply.f — contract requires '...' does not imply concrete requires '...'`; a postcondition violation reports `Contract subsumption failed: atom_ref(foo) passed to apply.f — concrete ensures '...' does not imply contract ensures '...'`.

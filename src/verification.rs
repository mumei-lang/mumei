...
/// Check that `concrete_atom.requires ∧ concrete_atom.ensures` implies `contract_ensures`.
///
...
fn check_contract_subsumption(
    ...
    //
    // NOTE: All parameters are currently created as Int. Atoms with f64/Str
    // parameters will silently skip the check (caught by expr_to_z3 Err).
    ...
    // Check: concrete_requires ∧ concrete_ensures ∧ ¬contract_ensures is UNSAT
    //        ⟺  (requires ∧ ensures) ⇒ contract_ensures (universally valid)
    solver.push();
    // Assert the concrete atom's requires so that the implication is
    // checked under the atom's precondition: (requires ∧ ensures) ⇒ contract_ensures.
    if concrete_atom.requires.trim() != "true" && !concrete_atom.requires.trim().is_empty() {
        let req_ast = parse_expression(&concrete_atom.requires);
        if let Ok(req_z3) = expr_to_z3(vc, &req_ast, &mut sub_env, None) {
            if let Some(req_bool) = req_z3.as_bool() {
                solver.assert(&req_bool);
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_subsumption_check_trivial_ensures_skipped() {
        // If either ensures is "true", subsumption is trivially satisfied.
        let cfg = Config::new();
        let ctx = Context::new(&cfg);
        let solver = Solver::new(&ctx);
        let int_sort = z3::Sort::int(&ctx);
        ...
        assert!(result, "trivial ensures should be skipped (returns true)");
    }

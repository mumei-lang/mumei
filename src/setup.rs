...
pub fn run(force: bool) {
    ...
    let z3_dir = toolchains_dir.join(format!(
        "z3-{}",
        z3_build.unwrap_or(Z3_LEGACY_BUILD).version
    ));
    if let Some(build) = z3_build {
        if let Err(e) = install_z3(&platform, &build, &toolchains_dir, &z3_dir, force) {
...
fn generate_env_script(
    ...
    // Upstream Z3 archives ship libz3.{so,dylib,a} in `bin`, not `lib`.

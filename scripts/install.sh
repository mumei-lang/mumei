...
#   curl -fsSL https://mumei-lang.github.io/mumei/install.sh | bash -s -- --version v0.6.19
...
Options:
  --version <VERSION>   Install a specific version (e.g. v0.6.19)
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            --version)
                shift
                if [ $# -eq 0 ]; then
                    err "--version requires an argument (e.g. --version v0.6.19)"
                fi
                REQUESTED_VERSION="$1"
                ;;
            --uninstall)

...
**Source references:** [NLAE guide](https://github.com/mumei-lang/mumei-agent/blob/develop/docs/NLAE_GUIDE.md), [NLAE integration](https://github.com/mumei-lang/mumei-agent/blob/develop/docs/NLAE_INTEGRATION.md), [mumei-agent roadmap P12](https://github.com/mumei-lang/mumei-agent/blob/develop/docs/ROADMAP.md#p12-nlae-integration-experimental--implemented), [latent protocol tests](https://github.com/mumei-lang/mumei-agent/blob/develop/tests/test_latent_protocol.py), [NLAE pipeline tests](https://github.com/mumei-lang/mumei-agent/blob/develop/tests/test_nlae_pipeline.py).

Mumei-agent includes experimental integration with Natural Language Autoencoders (NLAE) concepts, providing four opt-in capabilities:
...
**Latent Protocol for Agent Communication**: A standardized protocol for agent-to-agent communication using latent representations, with `lp-v2` versioned envelopes, `blake2b-128` semantic hashes, `hmac-sha256` authentication tags, optional AES-256-GCM payload encryption, and redacted audit logging.

**Multi-Agent Verification Workflow**: `MultiAgentOrchestrator` (`agent/nlae_multi_agent.py`) divides one specification between three specialized verification agents that collaborate over the latent protocol: a `generator` agent (generation plus `mumei verify --emit loss-vector`), a `counterexample` agent (Loss Vector driven self-correction rounds), and a `lean_escalation` agent (the mumei-lean fidelity check on the resulting proof certificate). Orchestration is deterministic — fixed roles, a fixed round order, and handoff bodies derived only from the specification and verifier output — so repeated runs of the same specification produce the same handoff semantic-hash sequence. Every handoff rides an existing latent envelope, so it is versioned, authenticated, optionally encrypted, and recorded in the redacted audit log; verdicts still come only from the verifier and the Lean bridge, and no verdict classification is added. The whole collaboration stays inside one distributed trace (`mumei.nlae.multi_agent` under the `mumei.nlae.pipeline` root span, `mumei.nlae.agent.<role>` per agent, `mumei.nlae.handoff` per handoff), and any failure inside the workflow degrades to the single P9-G pipeline with the fallback reason recorded.

These features are disabled by default and can be enabled via `ENABLE_LATENT_DEBUG`, `ENABLE_DENSE_PROPERTIES`, `ENABLE_LATENT_PROTOCOL`, and `ENABLE_NLAE_MULTI_AGENT` environment variables.
...
- NLAE integration (P12) for latent-space debugging, dense property generation, the latent inter-agent protocol, and the multi-agent verification workflow that splits one spec across `generator` / `counterexample` / `lean_escalation` agents over audited latent handoffs;
- P13 agent harness externalization with stage/module telemetry (`artifact_contract_passed`, `verification_gate`, `handoff_count`, `retry_class`, `intent_fidelity_status`), `--harness-profile` ablation runs, and harness stage evidence in `forge_log.json` and `proliferate --output-json`;
- proof-artifact distribution and tooling: the proof certificate chain, the package registry and `mumei publish`, `--allow-lean-verified` verified imports, the Homebrew-distributed standard-library certificate bundle, the `mumei verify-cert --strict` CI gate over that bundle, and LSP counterexample and Lean-escalation-state diagnostics consumed by the published VS Code extension;
...
- structured-concurrency ownership verification over `task_group:all` / `:any` (`Phase 1h-2`), rejecting concurrent double moves, moves racing a sibling's use, unsynchronised shared writes, parent uses after a child's move, and cancellation-dependent reads, plus Z3 constraints requiring every child's acquired resources to be released before the group completes;
...
- Lean escalation now has a typed translator v2 contract with obligation class taxonomy, CLI bundle/metrics output, and acceptance metrics; every one of the eight obligation classes (quantifier, finite-field, group-theory, crypto, arithmetic, smart-contract, RTGS, unknown) carries backing bridge lemmas, and `bridge_lemma_hash` is derived from that catalog so lemma-surface drift is detected mechanically. Thirteen live generated theorem paths reach `lean_verified` (the thirteenth, `predicate_guard_collapse`, is discharged by the widened tactic ladder under `tauto`; the tenth, `ff_mul_commutative`, discharges a swapped-operand finite-field equality through `MumeiLean.Algebra.ff_mul_comm_eq` under the `mumei_field` tactic cascade), and each escalation reports its `lake build` cost as `lean_solver_time_s`; obligations that still need a hand-written lemma remain marked rather than promoted;
...
- the natural-language extraction pipeline conservatively distinguishes missing requirements from underspecified intent and reports both through the fixed audit keys (`cross_validation_gaps` / `next_steps` / `verification_status`) without inventing clauses; the classification is still clause-level and lexical, so semantic gaps in prose that reads precise remain out of scope;
- arXiv-ready presentation will require replacing Mermaid diagrams with TikZ or figures;
- proof artifacts are distributed and surfaced end to end (certificate chain, registry publication, verified imports, Homebrew certificate bundle, LSP counterexample and Lean escalation diagnostics, VS Code extension, and a `mumei verify-cert --strict` CI gate that fails the standard-library certificate build on a drifted or unhashed certificate); the editor reports escalation state for the first undecided atom in a file plus every `lean_verified` atom recorded in a sibling certificate, so a file with several undecided atoms still needs successive edits to walk through them;
- the NLAE multi-agent verification workflow is opt-in and its agents are the existing generate / self-correct / Lean escalation stages under a deterministic latent-protocol handoff bus; independent agent policies (for example a counterexample agent that proposes its own probing specs) and concurrent agent execution remain out of scope;
- concurrency ownership obligations are decided syntactically, so they fail hard and never enter the Lean escalation path; each rejection is reported in the `diagnostics` array of `mumei verify --json` (`code`, `severity`, `message`, `tags`), while `warnings` stays advisory-only; interference reasoning for shared mutable state guarded by explicit synchronisation primitives, and codegen-side array-element capture in task bodies, remain open;
- benchmark evaluation suite (`benchmarks/run_benchmarks.py`) collects verification outcomes, solver times, and trusted atom ratios with time-series accumulation in `docs/BENCHMARK_RESULTS.md`. The `docs/ROADMAP.md` P16 expansion is implemented: `arithmetic`, `state_machine`, `concurrency`, and `domain_compliance` categories join `dafny_puzzles` and `svcomp_style`, growing the suite from 6 atoms across 2 categories to 105 atoms across 6 categories, and 20 counterexample (bug-catching) cases are all rejected by verification as expected (100% catch rate, with `success_rate` generalized to the expected-outcome match rate). Lean escalation solver time is collected per file as `lean_solver_time_s`; the 6 modular obligations in `benchmarks/arithmetic/finite_field_modular.mm` and `benchmarks/domain_compliance/modular_commitment.mm` really return Z3 `unknown`, so they are escalated to the `mumei-lean` bridge and recorded with a measured time and a per-category Lean discharge rate (6/6 `lean_verified`, 4 of them discharged by the automatic tactic search), while the channel still degrades to `SKIP` at zero cost when no Z3 `unknown` obligation arises or the bridge is absent. These results now feed back into the standard-library expansion pipeline: `--forge-feedback` emits a `mumei.benchmark_forge_feedback/v1` document whose per-category weakness score (expected-outcome match rate, counterexample catch rate, trusted ratio, plus Z3/Lean solver-time signals) biases the vStd forge / proliferate queue toward the stdlib domains the benchmark suite is weakest on.
...
1. broadening the automatic tactic search for `unknown` atoms that the obligation-class bridge lemma templates cannot discharge: the search itself is now implemented (`mumei-lean` `scripts/tactic_search.py`, `docs/LEAN_TRANSLATOR_SPEC.md` §12) as a deterministic, time-bounded candidate ladder whose adopted tactic is emitted as the generated proof and promoted to `lean_verified` only after the regenerated theorem really builds; the ladder now also covers propositional, list, order, and inductive goals (16 candidates; `tauto`, `mumei_list`, `mumei_order`, `mumei_induct`), and candidate order is learned from past runs as a deterministic permutation of the declared ladder, pinned in `mumei-lean` `data/tactic_search_history.json` (`mumei-lean.tactic_search_history/v1`) and recorded only for adoptions whose regenerated theorem really passed `lake build`; remaining work is broadening the learned history beyond the obligation classes the live paths exercise;
...
4. improving natural-language specification extraction with ambiguity reporting: the extraction pipeline now conservatively separates a *missing requirement* (the prose never mentions the subject and the extracted clause is trivial) from *underspecified intent* (the subject is described, or the prose is vague, but the clause was not pinned down), reporting the former as a `cross_validation_gaps` entry that blocks a `verified` verdict and the latter as a `next_steps` clarification request, never completing either with a guessed clause; remaining work is extending the classification beyond clause-level and lexical evidence;
5. strengthening concurrency and ownership proofs over MIR: coverage beyond `task_group:any` winner cancellation is now implemented as a structured-concurrency ownership phase (`Phase 1h-2`) that complements the sequential MIR move analysis — MIR lowers a `task_group` into a sequential chain of child bodies, so it models neither concurrent interleaving nor cancellation, and the added phase rejects concurrent double moves, moves that race a sibling's use, unsynchronised shared writes across siblings, parent uses of a value a child consumed, and parent reads of writes a cancelled `task_group:any` child may never have performed, with the group's Z3 encoding additionally requiring every child's acquired resource to be released before the group completes; captures of any declared type (arrays, structs, pointers, `f64`) are covered because movability comes from the type rather than the codegen marshalling path, with struct captures declared as `struct Name { ... }` (a `type Name = ...` declaration introduces a refinement type, not an aggregate). Remaining work is quantitative interference reasoning for shared mutable state behind explicit synchronisation, and codegen-side array-element capture in task bodies;
6. producing arXiv-ready figures and a quantitative evaluation suite;
7. integrating proof artifacts with package distribution, CI, and editor diagnostics: the distribution path is now implemented end to end — the proof certificate chain (`P5-A`), the package registry with `mumei publish` (`P5-B`), `--allow-lean-verified` verified imports (`P5-C`), the Homebrew standard-library certificate bundle (SI-5 Phase 3-C, `scripts/bundle_std_certs.py` plus `.github/workflows/generate-std-certs.yml`), and structured LSP counterexample diagnostics (`data.counterexample`) consumed by the published `mumei-lang.mumei` VS Code extension; the two remaining gaps are now closed as well — the LSP attaches the Lean escalation state of an atom under `data.lean_escalation` (`pending` with its `escalation_reason` while the obligation is routed to mumei-lean, `lean_verified` once a sibling proof certificate records the bridge as having discharged it) and the VS Code extension renders it as inline ghost text, and `.github/workflows/generate-std-certs.yml` gates the standard-library certificate bundle on `mumei verify-cert --strict`, which fails the build on a drifted atom, a certificate without a re-derivable `certificate_hash`, or a certificate whose source disappeared; remaining work is quantitative rather than structural: reporting escalation state for every atom in a file rather than the first undecided one;
8. validating the approach on larger safety-critical and compliance-heavy systems;
9. expanding NLAE capabilities for multi-agent verification workflows: this is now implemented as an opt-in workflow (`ENABLE_NLAE_MULTI_AGENT`, mumei-agent `agent/nlae_multi_agent.py`, `docs/NLAE_INTEGRATION.md` §P12-D) in which a `generator`, a `counterexample`, and a `lean_escalation` agent share one specification and hand work to each other over the existing latent protocol envelopes, so every handoff carries a versioned envelope, a semantic hash, an authentication tag, and a redacted audit entry; orchestration is deterministic (fixed roles and round order, reproducible handoff semantic-hash sequences), the collaboration stays inside one distributed trace, and any failure degrades to the single P9-G pipeline; remaining work is giving the agents independent policies and concurrent execution rather than a deterministic division of the existing pipeline stages;
10. completing P13 agent harness externalization with telemetry and ablation metrics: this is now implemented — `P13-B` aggregates `artifact_contract_passed`, `verification_gate`, `handoff_count`, `retry_class`, and `intent_fidelity_status` per stage and module in mumei-agent `agent/harness_metrics.py`, emits them in the same summary as the P8-G budget metrics, and exposes `--harness-profile basic|stateful|verifier|self_evolution|lean_fallback|full` ablation runs; `P13-C` adds harness stage evidence to `forge_log.json` and `proliferate --output-json` under the shared failure taxonomy and reads the canonical certificate metadata (`harness_contract`, `intent_fidelity`, `artifact_paths`, `budget_policy_fingerprint`); remaining work is accumulating ablation data over larger task populations rather than harness plumbing;
11. widening the benchmark-to-forge feedback channel: beyond the `mumei.benchmark_forge_feedback/v1` priority bias, proposal *generation* is now derived from benchmark weakness — weak categories (expected-outcome mismatch, missed counterexamples, trusted-atom pressure) emit deterministic new vStd proposals in an optional, backward-compatible `generated_proposals` field that `forge` / `proliferate` merge into the queue without displacing gap-analysis proposals; remaining work is closing the loop on whether the generated atoms measurably move the weak categories;
12. growing the benchmark evaluation suite further: it now spans 105 atoms across 6 categories, and the Lean escalation solver-time channel is exercised on obligations that really reach Z3 `unknown` — `benchmarks/arithmetic/finite_field_modular.mm` and `benchmarks/domain_compliance/modular_commitment.mm` contribute 6 modular obligations that Z3 answers `unknown`, of which 4 are discharged by the automatic tactic search, so `docs/BENCHMARK_RESULTS.md` records a measured `lean_solver_time_s` and a per-category Lean discharge rate instead of `SKIP`. The vStd forge / proliferate integration of the P16 results is now implemented: `benchmarks/run_benchmarks.py --forge-feedback <path>` emits a `mumei.benchmark_forge_feedback/v1` document, and `python -m agent forge|proliferate --benchmark-feedback <path>` maps each category's weakness score to a negative priority delta over its stdlib domains, recording the provenance in the run summary. The `docs/ROADMAP.md` P16 expansion itself (arithmetic, state-machine, concurrency, and domain-compliance categories pairing success cases with counterexample cases, plus `lean_solver_time_s` collection and per-category expected-outcome match rates, counterexample catch rates, solver times, and trusted ratios in `docs/BENCHMARK_RESULTS.md`) is now implemented, as is the P14 Cross-Validation Framework (V1-A through V1-E) across `mumei`, `mumei-agent`, and `mumei-demo`.

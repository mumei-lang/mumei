...
atom test_sorted_map_insert(keys: [i64], values: [i64], map_len: i64, map_cap: i64, key: i64, value: i64)
requires: map_len >= 0
    && map_cap > 0
    && map_len < map_cap
    && len(keys) >= map_cap
...
body: {
    smap::sorted_map_insert(keys, values, map_len, map_cap, key, value)
};

atom test_sorted_map_get(keys: [i64], map_len: i64, key: i64)
requires: map_len >= 0
    && forall(i, 0, map_len, keys[i] >= key || keys[i] < key);
ensures: result == 0 - 1 || (result >= 0 && result < map_len);
body: {
    smap::sorted_map_get(keys, map_len, key)
};

atom test_sorted_map_remove(keys: [i64], map_len: i64, key: i64)
requires: map_len > 0
    && len(keys) >= map_len
    && forall(i, 0, map_len - 1, keys[i] <= keys[i + 1]);
ensures: result >= 0
    ...
    && forall(i, 0, result - 1, keys[i] <= keys[i + 1]);
body: {
    smap::sorted_map_remove(keys, map_len, key)
};
...
atom test_sorted_map_insert_position(keys: [i64], map_len: i64, key: i64)
requires: map_len >= 0 && key >= map_len
    && forall(i, 0, map_len, keys[i] <= key || keys[i] > key);
...
atom test_sorted_map_is_sorted(keys: [i64], n: i64)
requires: n >= 0
    && forall(i, 0, n - 1, keys[i] <= keys[i + 1]);
ensures: result == 1;
body: {
    smap::sorted_map_is_sorted(keys, n)
};

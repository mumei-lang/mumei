...
#[cfg(unix)]
#[test]
fn lsp_suppresses_agent_diagnostics_for_verified_solidity() {
    let fixture_dir = unique_temp_dir("mumei-lsp-clean-sol-agent");
    let _ = fs::remove_dir_all(&fixture_dir);
    fs::create_dir_all(&fixture_dir).expect("create fake agent dir");
    write_fake_mumei_agent(&fixture_dir);

    let source_path = fixture_dir.join("GoodToken.sol");
    let source = "// SPDX-License-Identifier: MIT\npragma solidity ^0.8.0;\n\ncontract GoodToken {\n    function credit(uint256 balance, uint256 amount) public pure returns (uint256) {\n        return balance + amount;\n    }\n}\n";
    fs::write(&source_path, source).expect("write solidity source");
    let uri = format!("file://{}", source_path.display());
...
#[cfg(unix)]
#[test]
fn lsp_clears_agent_diagnostics_on_did_change_for_code() {
    let fixture_dir = unique_temp_dir("mumei-lsp-did-change-clear-agent");
    let _ = fs::remove_dir_all(&fixture_dir);
    fs::create_dir_all(&fixture_dir).expect("create fake agent dir");
    write_fake_mumei_agent(&fixture_dir);
    ...
    let bad_source = "def credit(balance, amount):\n    # bug\n    return balance\n";
    let clean_source = "def credit(balance, amount):\n    return balance + amount\n";
    fs::write(&source_path, bad_source).expect("write python source");
    let uri = format!("file://{}", source_path.display());
    ...
    assert!(
        per_message_diagnostics[0]
            .iter()
            .any(|d| d.get("source").and_then(Value::as_str) == Some("mumei-agent")),
        "bad open should emit mumei-agent diagnostics\nfirst diagnostics:\n{:#?}",
        per_message_diagnostics[0]
    );
    let changed = per_message_diagnostics
        .last()
    ...
    assert!(
        !changed
            .iter()
            .any(|d| d.get("source").and_then(Value::as_str) == Some("mumei-agent")),
        "clean change should clear mumei-agent diagnostics\nlast diagnostics:\n{changed:#?}"
    );
}

#[cfg(unix)]
#[test]
fn lsp_clears_agent_diagnostics_on_did_change_for_spec() {
    let fixture_dir = unique_temp_dir("mumei-lsp-did-change-clear-spec-agent");
    let _ = fs::remove_dir_all(&fixture_dir);
    fs::create_dir_all(&fixture_dir).expect("create fake agent dir");
    write_fake_mumei_agent(&fixture_dir);
...
atom ok(x: i64)
    ...
    body: { x }
"#;
    fs::write(&source_path, bad_source).expect("write mumei source");
    let uri = format!("file://{}", source_path.display());
    ...
    assert!(
        per_message_diagnostics[0]
            .iter()
            .any(|d| d.get("source").and_then(Value::as_str) == Some("mumei-agent")),
        "bad spec open should emit mumei-agent diagnostics\nfirst diagnostics:\n{:#?}",
        per_message_diagnostics[0]
    );
    let changed = per_message_diagnostics
        .last()
    ...
    assert!(
        !changed
            .iter()
            .any(|d| d.get("source").and_then(Value::as_str) == Some("mumei-agent")),
        "clean spec change should clear mumei-agent diagnostics\nlast diagnostics:\n{changed:#?}"
    );
}

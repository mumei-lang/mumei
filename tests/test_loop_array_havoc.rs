...
#[test]
fn loop_havoc_positive_claims_verify() {
    let output = mumei_verify("tests/test_loop_array_havoc.mm");
    let combined = format!(
        "{}\n{}",
...
#[test]
fn loop_havoc_rejects_stale_reads() {
    let output = mumei_verify("tests/test_loop_array_havoc_negative.mm");
    let combined = format!(
        "{}\n{}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)

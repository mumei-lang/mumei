...
### Follow-up（測定 2026-09-06、`cargo test --test test_concurrency` 36/36 passed、`cargo test --test test_run` 通過、`cargo test -p mumei-core lowering` 通過、`cargo tree --edges no-dev | grep -i opentelemetry` は空）

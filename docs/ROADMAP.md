...
## P31: task_group 並行意味論の bridge lemma 追加（C-1 群 2、hash lockstep）— 🚧 In Progress（`all`/`any` 完了・seq 中段形状のみ残）

**ステータス: 🚧 In Progress（2026-09-18 開始、2026-09-19 slice-2 で any 完了）**。P30 の C-1 棚卸しで「bridge lemma 追加が必要」と判明した唯一の形状群（`task_group: all` / `task_group: any`、benchmark 11 atom）を扱う。
...
- **残**: `task_group` が statement 列の中段に現れる形状（`let …; task_group:any {…}; tail` の `read_cancellable_write` 型）は依然 partial — 残存の seq セグメント対応が必要（なお当該 atom は `expected: FAIL` の反例であり live path 化の対象外）。

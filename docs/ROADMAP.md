...
- 群 3 B-4 実運用化（2026-09-19、mumei-agent）: これら atom は Z3 `unsat` のため `extract_unknown_atoms` の候補に乗らず AI 経路へ自動到達しなかった → `measure_lean_ai_proof.py --force-lean-atom <atom>`（repeatable）が bridge 入力 cert の写像でのみ `unknown` へ降格し、昇格マージもその写像で行う（未証明なら元の Z3 verdict へ復帰、出力回帰なし）。`run.json` に `force_lean_atoms` / `forced_atoms` / `lean_verified_forced` を出処として記録。**実走行（gpt-4o、ENABLE_LEAN_AI_PROOF=1）で `all_transactions_within_limit` が手動 `--external-proofs` 無しに AI 生成証明で `lean_verified` 到達**を実証（残り 2 atom は LLM 429 quota 停止、経路自体は到達済み）。

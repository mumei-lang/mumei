...
- **同レイアウト struct の名前的型検査**（`mumei-core/src/verification/support/nominal_types.rs`、Phase 0-nominal、測定 2026-09-06 で `cargo test --test test_concurrency` 39/39、`cargo test -p mumei-core` 483/483 passed）: LLVM の aggregate 型は構造的なので、上記 codegen 診断（scalar チャネルへの aggregate 送信拒否 / 混在型 `task_group:any` 拒否）は `struct Point { x, y }` と `struct Pair { a, b }` を区別できなかった。これは channel 固有ではなく通常の atom 呼び出し（`getx(p: Point)` に `Pair` を渡す）も frontend が受理していた frontend 全般の穴なので、verification パイプラインの Phase 0-units と同じ位置に AST 上の純粋な型検査を追加した。呼び出し引数の struct 型パラメータ、`chan<S>` への `send`、`task_group:any` の子の結果、atom の宣言戻り型に対して、式から構文的に確定する struct 型（struct literal / それを束縛した変数 / struct を返す call / `chan<S>` からの `recv`）が宣言と異なれば `TypeError` で拒否する。型検査であり証明義務ではないため `trusted` atom も対象。回帰テスト: `mumei-core` unit 6 件（call 引数 / 一致ケース / `send` / `task_group:any` / 戻り値 / `recv` 経由の型伝播）と `tests/test_concurrency.rs` 2 件（`Pair` を `chan<Point>` に `send`、`Point` 子と `Pair` 子の `task_group:any` が `mumei run` で拒否される）。std / examples 54 ファイルの `mumei verify` で誤検知なし。

...
| `edges[].is_consistent` | `boolean` / `null` | `true` = checked and consistent, `false` = checked and inconsistent, `null` = the pair was never examined by cross-spec analysis ("not checked" is not reported as "no mismatch") |

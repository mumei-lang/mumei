...
1. The atom has `z3_check_result == "lean_verified"` and unchanged `content_hash`.
...
3. Compute `content_hash` (content-hash v2; certificate `version` is written as `"1.1"`) and `proof_hash` (dependency-aware hash) for each atom

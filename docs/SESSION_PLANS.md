...
`mumei-core/src/verification.rs` の Phase 1h で、全ての `Rvalue::Use(Place::Local(..))` が
move として扱われている。Int/Bool/f64 などの Copy 型でも UseAfterMove 警告が出るため、
全ての違反が warning 止まりになっている。

`mumei-core/src/mir_analysis.rs` の `MirLinearityState` は `HashMap<Local, bool>` で
alive/consumed を追跡しているが、型情報を持っていない。

`mumei-core/src/mir.rs` の `LocalDecl` には `ty: Option<String>` フィールドが既にあるが、
MIR lowering 時に型情報が十分に伝搬されていない。

### 実装内容

1. **`mumei-core/src/mir.rs` の `LocalDecl` に型情報を確実に伝搬する**
   - `alloc_local()` の呼び出し元で、パラメータの型名を渡す
   - `alloc_temp()` で生成される一時変数にも型情報を付与

2. **`mumei-core/src/mir_analysis.rs` の `process_statement_for_moves()` を修正**
   - Copy 型の判定ヘルパー関数 `fn lookup_movability(local, locals) -> Movability` を追加（`movability_from_type()` を参照）
   - Copy 型の場合は `consume()` をスキップ

3. **`mumei-core/src/verification.rs` の Phase 1h を修正**
...
- `mumei-core/src/mir.rs`: LocalDecl, Movability, movability_from_type, alloc_local, lower_expr
- `mumei-core/src/mir_analysis.rs`: lookup_movability, process_statement_for_moves
- `mumei-core/src/verification.rs`: Phase 1h warning → error 昇格
...
`mumei-core/src/mir.rs` で、HirExpr::Match, AtomRef, CallRef, Async, Await, Task, TaskGroup, Lambda が
プレースホルダーに変換されていた。
...
- `mumei-core/src/mir.rs`: lower_expr 関数の各ケース
- `mumei-core/src/mir.rs`: MirConstant に FuncRef variant を追加
...
- `mumei-core/src/mir.rs`: LowerCtx, lower_expr (IfThenElse), lower_stmt (While)
...
`mumei-core/src/mir_analysis.rs` の TODO: SwitchInt discriminant 変数は terminator で
...
- `mumei-core/src/mir_analysis.rs`: `insert_drops` 関数の拡張
...
- `mumei-core/src/verification.rs`: verify_effect_params, VCtx, Sort-aware timeout
...
- `mumei-core/src/parser/ast.rs`: EffectDef.parent の型変更, Effect.negated 追加
- `mumei-core/src/parser/item.rs`: effect 定義パーサー, effects リストパーサー
- `mumei-core/src/parser/token.rs`: Pipe トークン追加
- `mumei-core/src/verification.rs`: get_effect_ancestors, is_subeffect, verify_effect_consistency
...
- `mumei-core/src/parser/token.rs`: Chan, Send, Recv, Cancel トークン
- `mumei-core/src/parser/lexer.rs`: キーワード認識
- `mumei-core/src/parser/ast.rs`: ChanSend, ChanRecv, Cancel variants
- `mumei-core/src/parser/expr.rs`: send/recv パーサー、cancel 文
- `mumei-core/src/hir.rs`: HirExpr::ChanSend, HirExpr::ChanRecv, HIR lowering
- `mumei-core/src/ast.rs`: Monomorphizer 更新
- `mumei-emit-llvm/src/codegen.rs`: LLVM codegen
- `mumei-core/src/mir.rs`: MIR lowering

- `mumei-core/src/verification.rs`: Z3 検証、各種 collect 関数
...
- `mumei-emit-llvm/src/codegen.rs`: enum_llvm_type(), StructInit field types
...
- `mumei-core/src/verification.rs`: Perform handler, parse_constraint_to_z3_string, check_constant_constraint
...
- `mumei-core/src/inspect.rs` (新規): InspectReport 生成
- `mumei-core/src/proof_cert.rs` (新規): ProofCertificate 生成・検証

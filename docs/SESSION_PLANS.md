...
MIR の Move Analysis において、Copy 型（Int, Nat, Bool, f64）と Move 型を区別し、
...
2. **`src/mir_analysis.rs` の `process_statement_for_moves()` を修正**
   - Copy 型の判定ヘルパー関数 `fn is_copy_type(ty: Option<&str>) -> bool` を追加
...
- `src/mir.rs`: LocalDecl, alloc_temp, lower_expr
- `src/mir_analysis.rs`: is_copy_type, process_statement_for_moves
...
`EffectDef` は `parent: Option<String>` で単一親のみサポート。
エフェクト階層の解決は `get_effect_ancestors()` と `is_subeffect()` で行われている。

...
| Polymorphic `chan<T>` payload marshalling | ✅ Implemented (P25: the runtime slot stays `int64_t`; `send` bit-preserves the payload into it and `recv` restores the declared `T` — `bitcast` for `f64`, `ptrtoint` / `inttoptr` for `Str` / pointer-backed values. A payload whose type differs from the declared `T` is numerically converted to `T` first, following the same int / float promotion rule as arithmetic, so `send(ch, 3)` on a `chan<f64>` transports `3.0` instead of an integer bit pattern. Aggregates passed by value (struct / array / vector LLVM values) have no bit-preserving i64 encoding, so `send` copies the value into a `malloc`ed box and transports the box address with `ptrtoint`, following the `Str` / pointer pattern; `recv` does `inttoptr`, loads the aggregate, and `free`s the box exactly once, so each box has a single owner (the sender until `send`, the channel until `recv`, then the receiver) and the Phase 1h-2 ownership analysis is unchanged. Boxed sends go through `__mumei_chan_send_owned(i64, i64) -> i64` (same body as `__mumei_chan_send`, but reports whether the slot was filled) so a send dropped by task-group cancellation frees its own box; a boxed `recv` woken by cancellation gets slot `0` and yields a zeroed aggregate instead of dereferencing it. If `malloc` for the box returns null, the generated code branches to `__mumei_payload_box_alloc_failed(i64)` (`noreturn`: `[mumei runtime] fatal: ...` + `abort()`, the runtime's established fatal path) instead of storing through the null pointer. Array payloads (`chan<[i64]>`) are fat pointers into the owner's storage with no by-value encoding and are rejected with a codegen diagnostic, as is an aggregate sent through a scalar channel (e.g. a struct on `chan<i64>`, whose box address would otherwise be read back as data and never freed). Struct payloads are also checked nominally before codegen (Phase 0-nominal, [`nominal_types.rs`](../mumei-core/src/verification/support/nominal_types.rs)): LLVM aggregate types are structural, so a `Pair { a, b }` sent on `chan<Point>`, passed to a `Point` parameter, returned from a `-> Point` atom, or mixed with a `Point` child in `task_group:any` is rejected as a `TypeError` even though both structs share one layout; the check also runs for `trusted` atoms because it is a type check, not a proof obligation. See [`box_payload_to_i64` / `unbox_payload_from_i64`](../mumei-emit-llvm/src/codegen/lowering.rs). Remaining constraints: the box is a shallow copy, so an aggregate containing pointers (e.g. a struct with a `Str` field) transports the pointer, not the pointee; a message still sitting in a channel slot at process exit is owned by the channel and is only reclaimed by exit.) |
| Non-i64 task body results at `join` | ✅ Implemented (P25 follow-up: the pthread result slot stays `i64`, but the wrapper stores the body result with the same `box_payload_to_i64` as `send` — `bitcast` for `f64`, `ptrtoint` for pointers, heap box for aggregates — instead of coercing non-int results to `0`. `PendingTask` records the body's LLVM type and `join` restores it with `unbox_payload_from_i64`. For `task_group:any` only the winner's slot is consumed by the parent, so a losing child `free`s its own boxed result inside the wrapper (`release_boxed_payload`) and no box is leaked or freed twice. See [`emit_task_join_only`](../mumei-emit-llvm/src/codegen/task_runtime.rs).) |

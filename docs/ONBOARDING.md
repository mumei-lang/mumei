...
- LSP (`mumei lsp`) を使い、エディタ上で diagnostics、hover、補完、定義ジャンプを確認する。`.mm` 内の `/// spec:` は `mumei-agent` の `spec_health_issues` として該当コメント行に出し、`.py` / `.rs` / `.ts` / `.tsx` / `.go` では `verification_violations` / `cross_validation_gaps` をインライン表示する。
- REPL (`mumei repl`) を使い、小さい式や atom をインタラクティブに検証する。自然言語仕様は `:verify-spec <path|inline>`、他言語コードは `:verify-code <path>` で `mumei-agent` の `spec_health_issues` / `verification_violations` / `verification_status` / `cross_validation_gaps` / `next_steps` を確認できる。

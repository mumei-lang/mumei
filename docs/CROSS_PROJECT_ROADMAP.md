...
| R-5 | P25 残課題 | 非 i64 task join 結果の codegen | ✅ 完了（2026-09-06）: task body の結果が `f64` / ポインタ / aggregate の場合の join 経路を `send` / `recv` と同じビット保存変換（`box_payload_to_i64` / `unbox_payload_from_i64`）へ揃えた | `cargo test --test test_concurrency` 30/30（join 型別ケース 4 件追加）。runtime helper シグネチャは i64 固定のまま |
...
| R-9 | P25 残課題 | 値渡し aggregate payload の codegen | ✅ 完了（2026-09-06）: struct を値で `send` / task 結果に載せる経路を boxed payload（`malloc` した box のアドレスを `ptrtoint` / `inttoptr` で i64 スロットへ搬送、受信 / join 側で `free`）で実装。runtime ABI 変更は不要だった | `cargo test --test test_concurrency` 30/30（struct 値渡し send/recv 2 件追加）。制約（shallow copy）は `docs/CONCURRENCY.md` に明記 |

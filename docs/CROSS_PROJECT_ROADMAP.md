...
| 🔧 | Priority 25: 意味モデル移行と AI 主体 Lean 証明生成 — 実行計画 | mumei-agent + mumei-lean + mumei + papers | In progress（棚卸し 2026-09-13。Track A: A-1〜A-5 ✅・A-6 第 1 弾 ✅ mumei-agent PR #570（残種別は Wave 4） / Track B: B-4・B-5・B-6 と B-0 の mumei-agent 側 ✅ PR #572、mumei-lean 側 B-1 / B-2 / B-3 ✅ PR #115 / #116、B-4 → B-2 経路接続 ✅・B-7 測定 ✅（2026-09-13、Wave 4 ✅ 完了）/ Track C: C-1 ✅ 分類済み（mumei P30 / P31 起票）・C-2 🔧 実装済み・レビュー中 + B-7 再測定 ✅（Wave 5、2026-09-13）。Wave 表は本書「Priority 25」節で実装実態に合わせて改訂済み） |
| 🔧 | Priority 26: mumei コンパイラ側残課題バックログの整理と対応順序 | mumei + mumei-agent + papers | In progress（棚卸し 2026-09-13: 群 1 R-1〜R-4 未実装・R-5 ✅・R-6 需要待ち / 群 2 R-7 ✅（2026-09-13、repair convergence `MEASURED`）・R-9 ✅・R-8 🔧 実装済み・レビュー中（= C-2、Wave 5、2026-09-13）・R-10 ✅ 分類済み（= C-1、mumei P30 / P31 起票）/ 群 3: 需要待ち R-11〜R-18。詳細は本書「Priority 26」節） |
...
| Priority 26: R-8（= C-2）/ R-10（= C-1） | 🔧 R-8 実装済み・レビュー中 / ✅ R-10 分類済み（2026-09-13） | 上記 C-1 / C-2 行 |
...
| C-2 🔧 実装済み・レビュー中（2026-09-13） | P10-D 有界低次数非線形算術 | 有界・低次数の非線形式を `nlsat` / `grobner` で Z3 側で先行処理し、無条件 Lean escalation を緩和する | Lean 送り件数そのものを減らす独立タスク。Track B とは非依存、B-7 の測定前に入れると比較が濁るため **B-7 の測定後** に着手 |
...
Wave 5（P25、現在）: C-2(=R-8) 🔧 実装済み・レビュー中 + B-7 再測定 ✅（同一 PR 群）, C-1(=R-10) ✅ 分類（P30、群 2 は P31 起票）, 残: B-1 続き（C-1 群 1 入力）, A-6 残り

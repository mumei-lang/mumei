...
| proof artifact の配布 / CI / エディタ診断統合 (paper Future Work #7) | mumei / homebrew-mumei | ✅ Implemented (残ギャップ 2 件を明示) | 実装済み: proof certificate chain（P5-A）、package registry + `mumei publish`（P5-B）、`--allow-lean-verified` verified import（P5-C）、homebrew 経由の std certificate bundle 配布（SI-5 Phase 3-C: `scripts/bundle_std_certs.py` + `.github/workflows/generate-std-certs.yml`）、LSP の構造化 counterexample 診断（`src/lsp.rs` の `data.counterexample`）と Marketplace 公開済み VS Code 拡張（`mumei-lang.mumei`）。残ギャップは 2 件に限定して切り出す: (1) エディタ側で `z3_check_result == "unknown"` atom の Lean escalation 状態（escalation 待ち / `lean_verified` 済み）を表示しない、(2) CI は std certificate を生成・アップロードするが、証明書が無効化されたときにビルドを失敗させる `mumei verify-cert` ゲートを持たない。paper の Future Work #7 と known-limitations 節はこの状態に同期済み |

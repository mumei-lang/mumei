...
| 契約式トランスレータ拡張 (mathlib4 active 利用) | ✅ Implemented | translator v2: 8 種の obligation class taxonomy (`quantifier_obligation`, `finite_field_obligation`, `group_theory_obligation`, `crypto_primitive_obligation`, `arithmetic_obligation`, `smart_contract_obligation`, `rtgs_obligation`, `unknown_obligation`)。`classify_obligation()` + `obligation_bridge_lemmas()` で各 escalated atom をクラス分類し、`MumeiLean/AdvancedPatterns.lean`, `Algebra.lean`, `Crypto.lean`, `Quantifiers.lean` の bridge lemma entry points にマッピング。`translator_version = mumei-lean-translator-ir-v2`, `bridge_lemma_hash` 更新済み |

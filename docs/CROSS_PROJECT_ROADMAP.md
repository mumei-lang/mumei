...
| 🔧 | Priority 25: 意味モデル移行と AI 主体 Lean 証明生成 — 実行計画 | mumei-agent + mumei-lean + mumei + papers | In progress（棚卸し 2026-09-13。Track A: A-1〜A-6 ✅ mumei-agent PR #570 / Track B: B-4・B-5・B-6 と B-0 の mumei-agent 側 ✅ PR #572、mumei-lean 側 B-1 / B-2 / B-3 は proposed、B-7 未着手 / Track C: C-1 / C-2 未着手。Wave 表は本書「Priority 25」節で実装実態に合わせて改訂済み） |
...
| 項目 | 現況 | 根拠 |
|---|---|---|
| Track A: A-1〜A-6 | ✅ Implemented | mumei-agent PR #570（`agent/dataflow_facts.py` / `tree_sitter_extract.extract_statements` / `tests/test_dataflow_facts.py`）。mumei-agent `docs/ROADMAP.md` P14-B「意味モデル化 ✅ Implemented (stage 3)」 |

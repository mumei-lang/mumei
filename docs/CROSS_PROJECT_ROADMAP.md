...
| trusted atom の削減 (Z3 Array store 追跡) | mumei | ✅ Complete | `std/list.mm::insertion_sort`, `tests/test_array_store.mm::test_array_store_loop`, `tests/test_verified_sort.mm::verify_noop_sort` / `verify_insertion_sort_skeleton` から `trusted` を除去 (PR #165 系列)。MIR move 解析の数値リテラル型推論 + `if/else` path 条件伝播 + ensures 内 `forall` の `arr[idx]` E-matching pattern 強化 + `smt.mbqi` の自動チューニング (`configure_array_quantifier_params` ヘルパに集約) で `forall + store` パターンを閉じる。Task 1-A (本 PR) で `mir.rs::infer_hir_ty()` の `Expr::ArrayAccess` ケースが `Some("i64")` を返すように整理（`let key = arr[i]` を `Movability::Copy` 扱いにして二重 while 内側ループでの MIR move 解析 false-positive を解消）し、`std/list.mm::verified_insertion_sort` / `verified_merge_sort` の `trusted` を完全に外した。`std/container/bounded_array.mm`, `std/container/verified_vector.mm` および `std/list.mm` 全体が trusted-free に到達 |

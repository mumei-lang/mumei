...
| ベンチマーク評価スイート拡張 (P16) | mumei / mumei-agent / mumei-lean | 🔧 Planned | mumei-local な実装チェックポイントは `docs/ROADMAP.md` P16。arithmetic / state_machine / concurrency / domain_compliance の 4 カテゴリを追加（成功例＋「検証で捕捉されるべきバグを含む反例ケース」）、総 atom 数 6→≥60・カテゴリ数 2→≥6、Lean escalation の solver 時間を収集対象化。ベンチマーク結果は mumei-agent の LLM 生成コード検証成功率測定と mumei-lean の escalation 成功率計測に接続する。canonical harness 語彙（`harness_contract` / `artifact_paths` / `lean_verified`）と既存優先順位を保持し、競合する priority order は導入しない |

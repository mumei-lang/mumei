...
**実装エビデンス**:
- ✅ **Task 1 — 配布物**: `scripts/bundle_std_certs.py` が v1.1 bundle の `artifact_paths`、Lean provenance、atom summary を生成し、`.github/workflows/release.yml` が `std/certs/` と bundle を Unix / Windows 配布物に同梱して unpack 後の全 certificate を `mumei verify-cert --strict` で再検証する。`scripts/homebrew/mumei.rb` と `scripts/generate_formula.py` も `MUMEI_PROOF_CERTS` と再検証手順を公開する。
- ✅ **Task 2 — 常時 CI**: `.github/workflows/stdlib-proof-gate.yml` が証明書生成、全 certificate の strict gate、bundle 生成、`scripts/generate_stdlib_metrics.py --check`、`scripts/check_proof_bundle_drift.py` を実行し、`std/certs/` と `std-proof-bundle.json` を artifact として保存する。`tests/test_check_proof_bundle_drift.py` が module / atom / translator / artifact drift を固定する。
...
- `.github/workflows/generate-std-certs.yml` / `.github/workflows/stdlib-proof-gate.yml` / `scripts/generate_stdlib_metrics.py` / `scripts/bundle_std_certs.py` / `scripts/check_proof_bundle_drift.py`
- `tests/test_check_mcp_tool_contract.py` / `tests/test_check_proof_bundle_drift.py` / `docs/MCP_TOOL_CONTRACT.md`

...
- ✅ **回帰なし**: `cargo test -p mumei-core registry` 8/8 passed（`registry.json` スキーマの後方互換とバージョン選択）。`cargo tree --edges no-dev` の出力に opentelemetry は現れない。

...
| 🔧 | Priority 25: 意味モデル移行と AI 主体 Lean 証明生成 — 実行計画 | mumei-agent + mumei-lean + mumei + papers | In progress（棚卸し 2026-09-13。Track A: A-1〜A-5 ✅・A-6 第 1 弾 ✅ mumei-agent PR #570（残種別は Wave 4） / Track B: B-4・B-5・B-6 と B-0 の mumei-agent 側 ✅ PR #572、mumei-lean 側 B-1 / B-2 / B-3 ✅ PR #115 / #116、B-4 → B-2 経路接続 ✅・B-7 測定 ✅（2026-09-13、Wave 4）/ Track C: C-1 / C-2 未着手。Wave 表は本書「Priority 25」節で実装実態に合わせて改訂済み） |
| 🔧 | Priority 26: mumei コンパイラ側残課題バックログの整理と対応順序 | mumei + mumei-agent + papers | In progress（棚卸し 2026-09-13: 群 1 R-1〜R-4 未実装・R-5 ✅・R-6 需要待ち / 群 2 R-7 ✅（2026-09-13、repair convergence `MEASURED`）・R-9 ✅、R-8 / R-10 は Priority 25 Wave 連動 / 群 3: 需要待ち R-11〜R-18。詳細は本書「Priority 26」節） |
...
| 4（現在） | B-4 を mumei-lean B-2 経路へ接続 ✅（`bridge.py --external-proofs` に tactic を渡し、B-3 `--failure-report` の構造化 JSON を feedback に使う、2026-09-13）/ A-6（残り、未着手） | B-1（続き）/ B-2 敵対行列拡充 ✅（SKILL.md §4） | B-7 ✅ + R-7 ✅（2026-09-13、同一 PR 群。repair convergence `SKIP` → `MEASURED`: 20 counterexample ファイル 24 atom、収束 10（41.67%）、平均 1.33 attempts、Ollama `qwen2.5-coder:3b`）| 進行中。残: A-6 残種別、B-1 続き。B-7 / R-7 の再測定は OpenAI 互換 API でも同一経路（`--ai-proof-cert-dir` / `--repair-cert-dir`）で可能 |
...
Wave 4（P25、現在）: B-4 の B-2 経路接続 ✅, B-7 + R-7 ✅（同一 PR 群、2026-09-13）, 残: B-1 続き, A-6 残り, C-1(=R-10)
Wave 5（P25）:    C-2(=R-8)   ※ B-7 測定値確定済みのため着手可（別 PR）
